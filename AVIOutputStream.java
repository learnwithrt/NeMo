 ...
 * @version $Id: AVIOutputStream.java 188 2012-03-28 14:03:19Z werner $
 */
public class AVIOutputStream extends AbstractAVIStream {
    ...
    public int addVideoTrack(String fccHandler, long scale, long rate, int width, int height, int depth, int syncInterval) throws IOException {
        ...
        VideoTrack vt = new VideoTrack(tracks.size(), fccHandler,//
                ...
                VideoFormatKeys.EncodingKey, fccHandler,
                VideoFormatKeys.DataClassKey, byte[].class,
                VideoFormatKeys.WidthKey, width, VideoFormatKeys.HeightKey, height, VideoFormatKeys.DepthKey, depth,
                VideoFormatKeys.FixedFrameRateKey, true,
    ...
    public int addAudioTrack(int waveFormatTag, //
        ...
        t.format = new Format(AudioFormatKeys.MediaTypeKey, FormatKeys.MediaType.AUDIO,
                AudioFormatKeys.MimeTypeKey, AudioFormatKeys.MIME_AVI,
                AudioFormatKeys.EncodingKey, RIFFParser.idToString(waveFormatTag),
                AudioFormatKeys.SampleRateKey, Rational.valueOf(afSampleRate),
                AudioFormatKeys.SampleSizeInBitsKey, sampleSizeInBits,
                AudioFormatKeys.ChannelsKey, numberOfChannels,
                AudioFormatKeys.FrameSizeKey, frameSize,
                AudioFormatKeys.FrameRateKey, Rational.valueOf(afSampleRate),
                AudioFormatKeys.SignedKey, sampleSizeInBits != 8,
    ...
    /** Returns the contents of the extra track header. Returns null if the header
     *  is not present.
     * <p>
     * Note: this method can only be performed before media data has been written
     * into the tracks.
     * 
     * @param track
     * @param fourcc
    ...
    private void writeProlog() throws IOException {
        ...
        for (Track tr : tracks) {
            ...
            for (RIFFChunk c : tr.extraHeaders) {
                DataChunk d = new DataChunk(RIFFParser.idToString(c.getID()),
                        c.getSize());
            ...
            if (tr.name != null) {
                ...
                DataChunk d = new DataChunk("strn",
                        data.length);
                ImageOutputStream dout = d.getOutputStream();
                dout.write(data);
                d.finish();
    ...
    private void writeEpilog() throws IOException {
        ...
        for (Track tr : tracks) {
            ...
            if (tr instanceof VideoTrack) {
                ...
                d.writeInt(vf.get(VideoFormatKeys.WidthKey)); // width
                d.writeInt(vf.get(VideoFormatKeys.HeightKey)); // height
                d.writeShort(1); // planes
                d.writeShort(vf.get(VideoFormatKeys.DepthKey)); // bitCount

                String enc = vf.get(VideoFormatKeys.EncodingKey);
                if (enc.equals(VideoFormatKeys.ENCODING_AVI_DIB)) {
                    d.writeInt(0); // compression - BI_RGB for uncompressed RGB
                } else if (enc.equals(VideoFormatKeys.ENCODING_AVI_RLE)) {
                    ...
                    d.writeInt(typeToInt(vt.format.get(VideoFormatKeys.EncodingKey))); // compression
                }

                if (enc.equals(VideoFormatKeys.ENCODING_AVI_DIB)) {
                    d.writeInt(0); // sizeImage
                } else {
                    if (vf.get(VideoFormatKeys.DepthKey) == 4) {
                        d.writeInt(vf.get(VideoFormatKeys.WidthKey) * vf.get(VideoFormatKeys.HeightKey) / 2); // sizeImage
                    } else {
                        int bytesPerPixel = Math.max(1, vf.get(VideoFormatKeys.DepthKey) / 8);
                        d.writeInt(vf.get(VideoFormatKeys.WidthKey) * vf.get(VideoFormatKeys.HeightKey) * bytesPerPixel); // sizeImage
                    }
                }
